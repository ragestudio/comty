...
export default async (sampleRate) => {
	...
	return await new Promise((resolve) => {
		app.layout.modal.confirm({
			headerText: "Change sample rate",
			descriptionText: `To change the sample rate, the app needs to be reloaded. Do you want to continue?`,
			onConfirm: () => {
				try {
					...
					app.message.error(
						`Failed to change sample rate, ${error.message}`,
					)
					return resolve(null)
				}
			},
