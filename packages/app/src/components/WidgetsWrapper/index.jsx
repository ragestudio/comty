...
class WidgetComponent extends React.Component {
	...
	render() {
		...
		try {
			...
			return <div className="widget_item">Invalid widget</div>
		}
	}
}
	...
	return lodable(
		async () => {
			try {
				...
				console.log(
					`🔄 Generating widget [${virtualModule.manifest.name}]`,
				)
				...
				if (RenderComponent.storagedStateKey) {
					...
					const defaultValue = await storagedStateEngine.getState(
						RenderComponent.storagedStateKey,
					)
					...
					ctx["storagedState"] = class {
						...
						static setState = async (value) => {
							return await storagedStateEngine.setState(
								RenderComponent.storagedStateKey,
								value,
							)
						}
						...
						static getState = async () => {
							return await storagedStateEngine.getState(
								RenderComponent.storagedStateKey,
							)
						}
					}
				...
				return () => (
					<div className="widget_item">Error loading widget</div>
				)
			}
		},
		{
