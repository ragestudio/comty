...
export const UsernameStepComponent = (props) => {
	...
	const [validLength, setValidLength] = React.useState(
		props.currentValue ? true : null,
	)
	const [validCharacters, setValidCharacters] = React.useState(
		props.currentValue ? true : null,
	)
	...
	React.useEffect(() => {
		...
		const timer = setTimeout(async () => {
			...
			const request = await AuthModel.availability({ username }).catch(
				(error) => {
					app.message.error(
		...
		<div className="register_form_step_content">
			...
			<div className="usernameValidity">
				<div className="check">
					{renderIndicator(
						validLength,
						"At least 3 characters / Maximum 64 characters",
					)}
				</div>
				<div className="check">
...
export default {
	...
	description: () => (
		<div>
			<p>
				Enter your username you gonna use for your account, its used to
				access to your account and give a easy name to identify you.
			</p>
			<p>
				You can set a diferent public name for your account after
				registration.
			</p>
		</div>
	),
