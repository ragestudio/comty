...
const DroppableField = (props) => {
	...
	return (
		<div
			className={classnames("droppableField", {
				["collapsed"]: collapsed,
			})}
		...
		>
			...
			<div className="inline_field">{props.header}</div>

			<div className="field_body">{props.children}</div>
		</div>
	)
}
	...
	return (
		...
		>
			...
			<div className="inline_field">
				...
				<div className="field_value">
					<p>{props.state.followersCount}</p>
				</div>
			</div>
				...
				<DroppableField
					header={
						<>
							...
							<div className="field_value">
								<p>{props.state.user?.badges.length}</p>
							</div>
						</>
					}
				>
