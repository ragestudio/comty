...
const WidgetsManager = () => {
	const [loadedWidgets, setLoadedWidgets] = React.useState(
		app.cores.widgets.getInstalled() ?? [],
	)
	...
	return (
		<div className="widgets-manager">
			<h1>Widgets</h1>

			<div className="widgets-manager-list">
				{Array.isArray(loadedWidgets) &&
					loadedWidgets.map((manifest) => {
						return (
							<React.Fragment>
								<WidgetItemPreview
									manifest={manifest}
									onRemove={() => {
										app.cores.widgets.uninstall(
											manifest._id,
										)
									}}
									onInstall={() => {
