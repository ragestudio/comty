...
function parseBasicAuth(auth) {
	...
	let decoded = Buffer.from(parts[1], "base64").toString("utf8")
	let colon = decoded.indexOf(":")
...
export default defineRoute<API>()({
	useContexts: ["server", "redis"] as const,
	fn: async (req, res, ctx) => {
		...
		if (!req.headers["authorization"].startsWith("Basic")) {
			throw new OperationError(
				401,
				"Invalid authorization type. Use Basic.",
			)
		}
		...
		const existMember = await ctx.redis.client.hexists(redis_id, "radio_id")

		if (data.online) {
			await global.redis.hset(redis_id, {
				...data,
		...
		if (!data.online && existMember) {
			await ctx.redis.client.hdel(redis_id)
		}

		console.debug(`Broadcasting radio [${data.radio_id}]`, data)

		ctx.server.engine.ws.senders.toTopic(
			`radio:${data.radio_id}`,
			"update",
			data,
		)
