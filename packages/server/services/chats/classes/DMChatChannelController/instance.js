...
export default class DMChatChannel extends ChatChannel {
	...
	onWrite = async (user, message) => {
		...
		let [senderActivity, receiverActivity] = await Promise.all([
			this.ActivityModel.findOne(senderActivityQuery),
			this.ActivityModel.findOne(receiverActivityQuery),
		])

		if (!senderActivity) {
			senderActivity = this.ActivityModel.obj({
				...senderActivityQuery,
		...
		if (!receiverActivity) {
			receiverActivity = this.ActivityModel.obj({
