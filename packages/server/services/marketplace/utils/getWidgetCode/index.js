import Widget from "@db_models/widget"
import compileWidgetCode from "@utils/compileWidgetCode"
import path from "path"

export default async (widgetId, { useCache = true, origin = null }) => {
	if (!widgetId) {
	...
	if (useCache) {
		widgetCode = await global.redis.get(
			`${origin}:widget:${widgetId}@${requestedVersion}}`,
		)
	}
	...
	if (!widgetCode) {
		try {
			widgetCode = await new Promise(async (resolve, reject) => {
				await global.storage.getObject(
		...
		try {
			...
			widgetCode = await compileWidgetCode(
				widgetCode,
				widget.manifest,
				finalOrigin,
			)
