...
export default class DesktopCapturerLinux {
	constructor(main) {
		...
		console.log("[desktopcapturer] Starting DesktopCapturer module [linux]")
		...
		if (!this.pulseaudio) {
			throw new Error(
				"[desktopcapturer] pulseaudio module not found, needed for DesktopCapturer",
			)
		}

		if (!this.pipewire) {
			throw new Error(
				"[desktopcapturer] pipewire module not found, needed for DesktopCapturer",
			)
		}
	...
	async initialize() {
		try {
			...
			if (!this.audioLoopback) {
				throw new Error(
					"[desktopcapturer] audioLoopback device not found, needed for DesktopCapturer",
				)
			}

			if (!this.audioRemap) {
				throw new Error(
					"[desktopcapturer] audioRemap device not found, needed for DesktopCapturer",
				)
			}
	...
	handlePulseClientId = async (clientId) => {
		...
		if (isChild) {
			console.log(
				"[desktopcapturer] The new pulse client is a child process, so skipped.",
			)
			return false
		}
		...
		if (!node) {
			console.error(
				"[desktopcapturer] Client can be linked to the loopback node, but cannot find the pw node",
			)
	...
	linkNodeToLoopback = async (node) => {
		for await (const [index, port] of node.ports.entries()) {
			...
			console.log(
				`[desktopcapturer] Linking pipewire port [${originPortId}] -> [${targetPortId}]`,
			)
